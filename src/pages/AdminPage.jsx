...
function DiagramView({ todayRes, tables, onEditReservation, onRefresh }) {
  ...
  const doUnmerge = async (r) => {
    // Find the absorbed reservation for this table/date/time
    const absorbed = todayRes.find(a => a.is_absorbed && a.table_id === r.table_id && a.time === r.time)
    if (absorbed) {
      await updateReservation(absorbed.id, { is_absorbed: false })
    }
